	...
	HighlightWithinTextarea.prototype = {
		...
		renderMarks: function(boundaries) {
			...
			input = input.replace(/\{\{hwt-mark-start\|(\d+)\}\}/g, function(match, submatch) {
				let startPos = boundaries[submatch].index;
				let className = 'error_' + startPos;
				
				return '<mark id=\"' + className + '\"class="clickable' + className + '">';
			});
		...
		xxx: function(){	//Hàm xử lý khi div-1 được scroll
			let scrolltop = $(this).scrollTop();	
			$('#text-area-1').scrollTop(scrolltop);	//Cho div-2 scroll đồng nhất với div 1
		},
